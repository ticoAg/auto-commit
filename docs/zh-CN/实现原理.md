...
本文面向中文团队，系统说明 `auto-commit` 的整体架构、关键流程与设计取舍，帮助二次开发与排障。
...
- 配置：仅支持 `~/.auto-commit/config.yml`（YAML）

## 总体架构
- 可执行入口：`bin/auto-commit`（调用 `src/auto-commit.js` 导出的 `main()`）
...
5) 提交与推送
   - 默认自动 `git add .`；
   - `--dry-run`：仅生成不提交；
   - `--save-template <name>`：在 dry‑run 下保存模板到 `~/.auto-commit/templates/`；
